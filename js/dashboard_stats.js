...
async function loadRecommendation(userId, session) {
    ...
    if (allRecent.length === 0) {
        // Ocultar si no hay historial absoluto
    ...
    titleEl.textContent = 'Buscando sugerencia...';
    artistEl.textContent = 'Analizando tu historial musical';
    reasonEl.textContent = 'Conectando con la IA de Spotify...';
    ...
    try {
        ...
        if (data.tracks && data.tracks.length > 0) {
            ...
            titleEl.textContent = 'Sin recomendaciones';
            artistEl.textContent = 'Escucha más canciones para darte ideas';
            reasonEl.textContent = '';
        }
    } catch(e) {
        console.error("Error cargando recomendación:", e);
        titleEl.textContent = 'Error de conexión';
        artistEl.textContent = 'No pudimos contactar a Spotify';
        reasonEl.textContent = '';
    }
}
