...
function getAvatarHTML(url, name, sizeClass = "w-20 h-20", isRobot = false) {
    if (isRobot) return `<div class="${sizeClass} rounded-full bg-orange-500/20 flex items-center justify-center text-4xl border-2 border-orange-500/30">🤖</div>`;
    if (url && url.trim() !== '') return `<img src="${url}" class="${sizeClass} rounded-full border-2 border-white/10 shadow-2xl object-cover">`;
...
async function renderArena(userId) {
    ...
    if (activeChallenges.length === 0) {
        container.innerHTML = `<div class="bg-card p-20 rounded-[3rem] w-full text-center border border-white/5"><p class="text-neutral-500 italic">Busca a un amigo para empezar el duelo.</p></div>`;
        return;
    }
    ...
    // FILTRADO DE TIEMPO (Solo desde que empezó el reto)
    const challengeStart = new Date(ch.start_date);
    const now = new Date();
    let filterStart = new Date();
    
    if (currentFilter === 'today') filterStart.setHours(0,0,0,0);
    else if (currentFilter === 'week') filterStart.setDate(now.getDate() - 7);
    else if (currentFilter === 'month') filterStart.setMonth(now.getMonth() - 1);

    // REGLA DE ORO: No contar nada antes de que existiera el reto
    const finalStartDate = new Date(Math.max(filterStart, challengeStart)).toISOString();

    const [myTime, friendTime] = await Promise.all([
        getListeningTimeInRange(userId, finalStartDate, now.toISOString()),
        getListeningTimeInRange(opponentId, finalStartDate, now.toISOString())
    ]);
    ...
    const winHistory = await getWinHistoryOptimized(userId, opponentId, ch.start_date);
        ...
        <div class="bg-card w-full rounded-[4rem] p-10 sm:p-16 border border-white/5 relative overflow-hidden shadow-2xl">
            <div class="flex items-center justify-center gap-8 sm:gap-16 mb-12">
                <div class="flex flex-col items-center gap-4">
                    <div class="relative">${userAvatar}${myMins >= friendMins && myMins > 0 ? '<span class="absolute -top-3 -right-3 bg-spotify-green text-black text-[9px] font-black px-3 py-1 rounded-full shadow-lg">LÍDER</span>' : ''}</div>
                    <div class="text-center"><p class="text-[10px] font-black uppercase text-spotify-green tracking-widest mb-1">Tú</p><p class="text-4xl font-black italic tracking-tighter">${myMins}<span class="text-xs font-bold opacity-20 ml-1">M</span></p></div>
                </div>
                <div class="text-xl font-black text-white/10 italic">VS</div>
                <div class="flex flex-col items-center gap-4">
                    <div class="relative">${friendAvatar}${friendMins > myMins ? '<span class="absolute -top-3 -right-3 bg-red-500 text-white text-[9px] font-black px-3 py-1 rounded-full shadow-lg">LÍDER</span>' : ''}</div>
                    <div class="text-center"><p class="text-[10px] font-black uppercase text-red-500 tracking-widest mb-1">${otherUser.display_name.split(' ')[0]}</p><p class="text-4xl font-black italic tracking-tighter">${friendMins}<span class="text-xs font-bold opacity-20 ml-1">M</span></p></div>
                </div>
            </div>

            <div class="space-y-4 mb-10">
                <div class="h-3 bg-black/40 rounded-full flex overflow-hidden p-0.5 border border-white/5">
                    <div class="h-full bg-spotify-green rounded-full transition-all duration-1000" style="width: ${myPercent}%"></div>
                    <div class="h-full bg-red-600 rounded-full transition-all duration-1000" style="width: ${friendPercent}%"></div>
                </div>
                <div class="flex justify-between px-2 text-[9px] font-black text-neutral-500 uppercase tracking-widest">
            ...
            <div class="flex flex-col items-center gap-8">
                <div class="w-full max-w-xs flex gap-1.5 h-1.5">
                    ${winHistory}
                </div>
                
                <div class="relative group">
                    <div class="absolute inset-0 bg-spotify-green/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <div class="relative py-4 px-10 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
                        <p class="text-center text-xs font-black italic tracking-[0.3em] ${myMins >= friendMins ? 'text-spotify-green' : 'text-red-500'}">
                            ${myMins >= friendMins ? '¡DOMINANDO LA ARENA!' : '¡RECUPERA EL LIDERATO!'}
                        </p>
                    </div>
                </div>
            </div>
...
async function getWinHistoryOptimized(userId, friendId, challengeStart) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (d < new Date(challengeStart)) {
            html += `<div class="win-line bg-transparent border border-white/5"></div>`;
        } else if (myD === 0 && friD === 0) {
            html += `<div class="win-line win-none"></div>`;
        } else if (myD >= friD) {
            html += `<div class="win-line win-user"></div>`;
        } else {
            html += `<div class="win-line win-friend"></div>`;
        }
    }
    return html;
}
...
async function loadPendingRequests(userId) {
    ...
    container.innerHTML = (requests || []).length === 0 ? '<p class="text-center text-neutral-600 text-xs py-10 italic">Sin solicitudes pendientes.</p>' : '';
    requests?.forEach(req => {
        const div = document.createElement('div');
        div.className = "flex items-center justify-between p-4 bg-black/20 rounded-2xl border border-white/5";
        div.innerHTML = `<div class="flex items-center gap-3"><img src="${req.creator.avatar_url || ''}" class="w-10 h-10 rounded-full border border-white/10"><span class="text-sm font-bold text-white">${req.creator.display_name}</span></div><button class="bg-spotify-green text-black text-[10px] font-black px-5 py-2.5 rounded-full">Aceptar</button>`;
...
async function loadActiveChallengesList(userId) {
    ...
    activeChallenges.forEach(ch => {
        ...
        const div = document.createElement('div');
        div.className = "flex items-center justify-between p-4 bg-black/20 rounded-2xl border border-white/5";
        div.innerHTML = `<div class="flex items-center gap-3"><img src="${otherUser.avatar_url || ''}" class="w-10 h-10 rounded-full"><div><p class="text-sm font-bold text-white">${otherUser.display_name}</p><p class="text-[8px] text-neutral-500 uppercase tracking-widest font-black">Duelo Activo</p></div></div><button class="text-[10px] font-black text-red-500/50 hover:text-red-500 uppercase">Terminar</button>`;
...
function renderSearchResults(users, currentUserId) {
    ...
    users.forEach(u => {
        const div = document.createElement('div');
        div.className = "flex items-center justify-between p-4 bg-black/20 rounded-2xl border border-white/5";
        div.innerHTML = `<div class="flex items-center gap-3"><img src="${u.avatar_url || ''}" class="w-10 h-10 rounded-full border border-white/10"><span class="text-sm font-bold text-white">${u.display_name}</span></div><button class="bg-spotify-green text-black text-[10px] font-black px-5 py-2.5 rounded-full">Retar</button>`;
        container.appendChild(div);
